...
export PATH=$PATH:/usr/local/jruby-$JRUBY_MAJOR_MINOR/bin
passenger-install-nginx-module --auto --languages ruby

ln -sf /opt/nginx/conf /etc/nginx
ln -sf /opt/nginx/logs /var/log/nginx
ln -sf /opt/nginx/sbin/nginx /usr/sbin/
...
if [[ -e /usr/local/jruby-$JRUBY_MAJOR_MINOR/bin/jruby ]]; then
